...
def load_mistral_model(base_model_name="mistralai/Mistral-7B-v0.3", lora_path="output/finetuned_lora"):
    ...
    어댑터를 기본 모델에 병합(merge)하여 text-generation pipeline에서 지원되는 모델로 변환
    이후, LangChain에서 사용할 수 있도록 HuggingFacePipeline으로 래핑하여 반환합니다.
    ...
    # LoRA 어댑터를 기본 모델에 병합하여, text-generation pipeline에서 지원하는 모델 형태로 변환
    #어댑터를 merge하여 일반 모델로 만들어 주므로, 이후 생성 시 LoRA 효과 반영
